...
function begin_play(){
	p1=document.getElementById("player1_id").value
	p2=document.getElementById("player2_id").value
	if(isEmpty(p1) || isEmpty(p2)){
		alert("Two player game, both the fields are manditory.")
	} else{
...
function play() {
	if(document.getElementById("player1_id").disabled==true){
		space=document.getElementById("move_text_id").value
		if(table_ids.includes(space) && space==document.getElementById(space).innerHTML){
			var x = "X"
			var o = "O"
			if(document.getElementById("turn_info").innerHTML=="Turn for: "+ x.bold()){
			...
			for(var i=0; i<3; i++){ 
				/*Check Verticles*/
				if(document.getElementById(table_ids[i]).innerHTML == document.getElementById(table_ids[i+3]).innerHTML && document.getElementById(table_ids[i]).innerHTML == document.getElementById(table_ids[i+6]).innerHTML){
					alert("Winner is: "+document.getElementById(table_ids[i]).innerHTML+"!")
					reset_play()
				}
				/*Check Horizontals*/
				else if(document.getElementById(table_ids[3*i]).innerHTML == document.getElementById(table_ids[3*i+1]).innerHTML && document.getElementById(table_ids[3*i]).innerHTML == document.getElementById(table_ids[3*i+2]).innerHTML){
					alert("Winner is: "+document.getElementById(table_ids[3*i]).innerHTML+"!")
					reset_play()
				}
				/*Now just the Two Diagonals*/
				else if(document.getElementById("B2").innerHTML == document.getElementById("A1").innerHTML && document.getElementById("B2").innerHTML == document.getElementById("C3").innerHTML){
					alert("Winner is: "+document.getElementById("B2").innerHTML+"!")
					reset_play()
				}
				else if(document.getElementById("B2").innerHTML == document.getElementById("C1").innerHTML && document.getElementById("B2").innerHTML == document.getElementById("A3").innerHTML){
					alert("Winner is: "+document.getElementById("B2").innerHTML+"!")
					reset_play()
				}
			}
			...
			alert("Invalid Entry. Please enter a value corresponding to an available space.")
		}
	}
	else{alert("The game has not started.")}
}
